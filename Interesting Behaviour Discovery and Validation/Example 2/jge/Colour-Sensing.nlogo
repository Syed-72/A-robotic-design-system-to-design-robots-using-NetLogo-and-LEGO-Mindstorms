...
Simply insert the name of your robot, send commands to robot by pressing buttons and check the colour displayed in Display window.

## THINGS TO NOTICE

The monitors may produce spurious results regarding colours such as colours which are not on the test-floor or wrong colour detection which is on the test-floor but robot is on a different colour. This unfortunately is a problem with the test-floor, a better quality of test-floor can be utilised to avoid this problem.

## THINGS TO TRY & EXTENDING THE MODEL

Try sensing some other colours which are not on the test floor, add their values by adding sliders and see if robot can detect them correctly.
