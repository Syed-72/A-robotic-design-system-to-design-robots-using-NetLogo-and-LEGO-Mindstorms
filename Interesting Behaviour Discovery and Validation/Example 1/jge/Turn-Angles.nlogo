;********************************************************
; FILENAME: Turn-Angles.nlogo
; DATE    : 14/08/2021, 12:12PM
; AUTHOR  : Syed Kaleem Aslam
;
; DESCRIPTION:
; -------------------------------------------------------
; A GUI for checking colour of the patches and to find the
; best colour threshold level to detect colours accurately.
; Observe the robot turn angles by selecting a variety of
; angles by using the slider to make sure the robot is
; taking the turn at desired/accurate turn angle.
;********************************************************
...
;check the colour of the patch robot is currently on
to ColourCheck

  if ROBOT::COLOUR-AHEAD-YELLOW? = true
  [
    ask patches
    [set pcolor yellow]
  ]
  if ROBOT::COLOUR-AHEAD-BLUE? = true
  [
    ask patches
    [set pcolor blue]
  ]
  if ROBOT::COLOUR-AHEAD-GREEN? = true
  [
        ask patches [
      set pcolor green]
  ]
  if ROBOT::COLOUR-AHEAD-RED? = true
  [
        ask patches [
      set pcolor red]
  ]
  if ROBOT::COLOUR-AHEAD-WHITE? = true
  [
        ask patches [
      set pcolor white]
  ]
...
387
execution-commands
ifelse (ROBOT::COLOUR-AHEAD-BLUE?) [ROBOT::LEFT 90][ROBOT::FORWARD] ifelse (ROBOT::COLOUR-AHEAD-RED?) [ROBOT::RIGHT 90][ROBOT::FORWARD]
1
1
String (commands)
...
276
Right Turn
ROBOT::RIGHT Right-Turn-Angle * 3
NIL
1
T
OBSERVER
...
Using the nxtlogo extension, the application connects to a robot of `robot-name` via blueooth and then runs the specified grammar on the device by continueously communicating. Also, user can send commands to check specific actions such as turn angles.

## HOW TO USE IT

Simply insert the name of your robot, send commands to robot to check the turn angles, it also displays features of the colour-sensing model which let you check the colour of the floor as well.

## THINGS TO NOTICE

The turn angles accuracy, robot taking turn is accurate or not?

## THINGS TO TRY & EXTENDING THE MODEL

Try different angles and create new testing floor with extra markings and try if the robot taking turns accurately, see how well robot performs for different angles.
...
Credit goes to William Faithful, the developer of the nxtlogo extension for his API and sample applications. Also, credit goes to Llifon Jones for updating the nxtlogo extension upto API 5.1.
@#$#@#$#@
default
true
0
